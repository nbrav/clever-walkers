...
public class QAgent : MonoBehaviour
{
	...
	/*disc_radii = new float[] {1f, 2f, 3f, 4f, 5f};
	sector_angles = new float[] {0.0f,
				     45.0f*Mathf.Deg2Rad,
				     90.0f*Mathf.Deg2Rad,
				     135.0f*Mathf.Deg2Rad,
	...
	for (int sector = 0; sector < num_sector*2; sector++)
		...
		if(collisioncell[cc_idx,4]>0.1f)
		{
		    phi.Add(cc_idx);
    ...
    void simulate_hippocampus()
    {
	    ...
	    if (vizPlaceCell) circlePlaceCell[pc_idx].UpdateMeshColor(new Color(placecell[pc_idx, 2], 1-placecell[pc_idx, 2], 0.0f, placecell[pc_idx, 2]));
        }
    }
